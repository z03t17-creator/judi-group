...
import { FieldMovementsDirectory } from "./field-movements-directory";

export default async function FieldMovementsPage() {
      ...
      <header className="flex flex-wrap items-start gap-3">
        ...
        <div className="min-w-0 flex-1 text-start">
          <p className="flex items-center gap-2 text-sm font-medium text-judi-800 dark:text-judi-200">
            ...
            {t("field.movementsTitle")}
          </p>
          <h1 className="text-xl font-bold tracking-tight text-fg sm:text-2xl">
            {t("field.movementsTitle")}
          </h1>
          <p className="mt-0.5 text-sm text-fg-muted">{t("field.movementsSubtitle")}</p>
        </div>
      </header>

      <FieldMovementsDirectory
        invoices={activity.invoices}
        collections={activity.collections}
        stockMoves={activity.stockMoves}
      />
    </main>
  );
}
