...
import { createFieldInvoiceAction } from "./actions";

export default async function FieldInvoicePage({
  searchParams,
}: {
  ...
  const [stores, products, inventories, discountRules] = await Promise.all([
    ...
    <main className="mx-auto w-full max-w-content space-y-4">
      <header className="flex flex-wrap items-start gap-3">
        ...
        <div className="min-w-0 flex-1 text-start">
          ...
          <p className="mt-0.5 text-sm text-fg-muted">{t("field.orderHint")}</p>
        </div>
      </header>
      ...
      <InvoiceForm
        action={createFieldInvoiceAction}
        surface="field"
